...
from SideBarEnhancements.lib.edit.Edit import Edit as Edit
from SideBarEnhancements.lib.hurry.filesize import size as hurry_size
...
class SideBarFindFilesPathContainingCommand(sublime_plugin.WindowCommand):
    def run(self, paths=[]):
        ...
        view.set_syntax_file('Packages/SideBarEnhancements/other/SideBar Results.hidden-tmLanguage')
...
class SideBarCutCommand(sublime_plugin.WindowCommand):
    ...
    def is_enabled(self, paths=[]):
        return CACHED_SELECTION(paths).len() > 0 and CACHED_SELECTION(paths).hasProjectDirectories() is False
...
class SideBarDuplicateCommand(sublime_plugin.WindowCommand):
    ...
    def is_enabled(self, paths=[]):
        return CACHED_SELECTION(paths).len() == 1 and CACHED_SELECTION(paths).hasProjectDirectories() is False
...
class SideBarProjectItemAddCommand(sublime_plugin.WindowCommand):
    ...
    def is_enabled(self, paths=[]):
        return CACHED_SELECTION(paths).hasDirectories() and CACHED_SELECTION(paths).hasProjectDirectories() is False
...
class SideBarOpenInBrowserCommand(sublime_plugin.WindowCommand):
    def run(self, paths=[], type=False, browser=""):
        ...
        if type is False or type == 'testing':
            type = 'url_testing'
